...
function adminInspectSku(maCanSoi, bu) {
  ...
  // Tìm theo mã ĐÃ CHUẨN HOÁ (trim) — sku_code là `text` thật trong Postgres
  // (không còn kiểu "ô tự đổi thành number" của Sheets), nhưng vẫn trim vì
  // khoảng trắng thừa hai đầu vẫn là lỗi dữ liệu có thật, độc lập với storage.
  var thay = readObjects_(SHEETS.PRODUCTS).filter(function (p) {
    return String(p.sku_code == null ? '' : p.sku_code).trim() === can;
  });

  if (!thay.length) {
    out.push('KHÔNG có dòng nào trong bảng Products có mã này (sau khi cắt khoảng trắng).');
    return psInLog_(out);
  }

  if (thay.length > 1) {
    out.push('CẢNH BÁO: có ' + thay.length + ' dòng cùng mã này — vi phạm khoá chính');
    out.push('sku_code, đáng lẽ Postgres đã chặn ngay lúc ghi. Báo lại nếu thấy dòng này.');
    out.push('');
  }

  thay.forEach(function (o) {
    out.push('--- Dòng Products, mã ' + o.sku_code + ' ---');
    ['sku_code', 'name', 'default_channel', 'is_active', 'avg_price', 'product_group_code'].forEach(function (cot) {
      out.push('  ' + (cot + '                    ').slice(0, 20) + JSON.stringify(o[cot]));
    });
  ...
  // (Bỏ phần so KIỂU dữ liệu số/chuỗi giữa Products và MonthlyForecastLines:
  // cả hai đều là cột `text` thật trong Postgres, không còn đường nào một
  // bảng lưu number còn bảng kia lưu string cho cùng 1 sku_code.)
  out.push('');
  out.push('--- Mã này xuất hiện thế nào trong MonthlyForecastLines ---');
  var ml = readObjectsWhere_(SHEETS.MONTHLY_LINES, 'sku_code', can);
  out.push('  số dòng dự báo: ' + ml.length);
  ...
  if (ml.length) {
    var theoDonVi = {};
    var vs = {}, cs = {};
    readObjectsWhere_(SHEETS.VERSIONS, 'id', uniqueValues_(ml.map(function (l) { return l.version_id; })))
      .forEach(function (v) { vs[String(v.id)] = v; });
    Object.keys(vs).length && readObjectsWhere_(SHEETS.CYCLES, 'id',
      uniqueValues_(Object.keys(vs).map(function (k) { return vs[k].cycle_id; })))
      .forEach(function (c) { cs[String(c.id)] = c; });
    ml.forEach(function (l) {
      var ver = vs[String(l.version_id)];
